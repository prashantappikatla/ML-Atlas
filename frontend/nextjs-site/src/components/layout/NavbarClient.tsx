'use client'

import { useState } from 'react'
import Link from 'next/link'
import { Menu, X, Brain } from 'lucide-react'
import { CATEGORIES } from '@/lib/categories'

export function NavbarClient() {
  const [mobileOpen, setMobileOpen] = useState(false)
  const [algosOpen, setAlgosOpen] = useState(false)

  return (
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80">
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-2 font-semibold text-slate-900">
          <Brain className="h-5 w-5 text-indigo-600" />
          <span>ML Atlas</span>
        </Link>

        {/* Desktop nav */}
        <nav className="hidden items-center gap-1 md:flex">
          {/* Algorithms dropdown */}
          <div className="relative">
            <button
              onClick={() => setAlgosOpen((v) => !v)}
              onBlur={() => setTimeout(() => setAlgosOpen(false), 150)}
              className="flex items-center gap-1 rounded-md px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 hover:text-slate-900"
            >
              Algorithms
              <svg
                className={`h-4 w-4 transition-transform ${algosOpen ? 'rotate-180' : ''}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
              </svg>
            </button>

            {algosOpen && (
              <div className="absolute left-0 top-full z-10 mt-1 w-64 rounded-lg border border-slate-200 bg-white py-2 shadow-lg">
                <Link
                  href="/algorithms"
                  className="block px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-slate-50"
                >
                  Browse All (278)
                </Link>
                <div className="my-1 border-t border-slate-100" />
                {CATEGORIES.map((cat) => (
                  <Link
                    key={cat.slug}
                    href={`/algorithms/${cat.slug}`}
                    className="flex items-center justify-between px-4 py-1.5 text-sm text-slate-600 hover:bg-slate-50 hover:text-slate-900"
                  >
                    <span>{cat.name}</span>
                    <span className="text-xs text-slate-400">{cat.count}</span>
                  </Link>
                ))}
              </div>
            )}
          </div>

          <Link
            href="/labs"
            className="rounded-md px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 hover:text-slate-900"
          >
            Labs
          </Link>
          <Link
            href="/progress"
            className="rounded-md px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 hover:text-slate-900"
          >
            Progress
          </Link>
          <Link
            href="/search"
            className="rounded-md px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 hover:text-slate-900"
          >
            Search
          </Link>

          <a
            href="https://github.com/prashantappikatla/ML-Atlas"
            target="_blank"
            rel="noopener noreferrer"
            className="ml-2 rounded-md p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-600"
            aria-label="GitHub"
          >
            <svg viewBox="0 0 24 24" className="h-5 w-5" fill="currentColor">
              <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" />
            </svg>
          </a>
        </nav>

        {/* Mobile burger */}
        <button
          onClick={() => setMobileOpen((v) => !v)}
          className="rounded-md p-2 text-slate-600 hover:bg-slate-100 md:hidden"
          aria-label="Toggle menu"
        >
          {mobileOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
        </button>
      </div>

      {/* Mobile menu */}
      {mobileOpen && (
        <div className="border-t border-slate-200 bg-white px-4 py-3 md:hidden">
          <Link
            href="/algorithms"
            onClick={() => setMobileOpen(false)}
            className="block py-2 text-sm font-medium text-slate-700"
          >
            Browse All Algorithms
          </Link>
          <div className="ml-3 mt-1 space-y-1">
            {CATEGORIES.map((cat) => (
              <Link
                key={cat.slug}
                href={`/algorithms/${cat.slug}`}
                onClick={() => setMobileOpen(false)}
                className="block py-1.5 text-sm text-slate-500"
              >
                {cat.name}
              </Link>
            ))}
          </div>
          <div className="mt-3 border-t border-slate-100 pt-3 space-y-1">
            {[
              { href: '/labs', label: 'Labs' },
              { href: '/progress', label: 'Progress' },
              { href: '/search', label: 'Search' },
            ].map(({ href, label }) => (
              <Link
                key={href}
                href={href}
                onClick={() => setMobileOpen(false)}
                className="block py-2 text-sm font-medium text-slate-700"
              >
                {label}
              </Link>
            ))}
          </div>
        </div>
      )}
    </header>
  )
}
